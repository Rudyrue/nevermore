...
import flixel.graphics.tile.FlxDrawNoteItem;
...
class FlxCamera extends FlxBasic
{
	...
	/**
	 * Pointer to current stack of draw items
	 */
	var _currentDrawStack:FlxDrawBaseItem<Dynamic>;
	...
	/**
	 * Draw note stack items that can be reused
	 */
	 static var _storageNoteHead:FlxDrawNoteItem;
	...
	@:noCompletion
	public function startNoteBatch(graphic:FlxGraphic, ?blend:BlendMode, smooth:Bool = false, customColoring:Bool = false)
	{
		var itemToReturn = null;
		var blendInt:Int = FlxDrawBaseItem.blendToInt(blend);

		if (_currentDrawItem != null
			&& _currentDrawItem.type == FlxDrawItemType.NOTE
			&& _headNote.graphics == graphic
			&& _headNote.blending == blendInt
		...
		if (_storageNoteHead != null)
		{
			itemToReturn = _storageNoteHead;
			var newHead = _storageNoteHead.nextTyped;
			itemToReturn.reset();
			_storageNoteHead = newHead;
		}
		else
		{
			itemToReturn = new FlxDrawNoteItem();
		}
		...
		itemToReturn.colored = customColoring;

		itemToReturn.nextTyped = _headNote;
		_headNote = itemToReturn;
	...
	public function startQuadBatch(graphic:FlxGraphic, colored:Bool, hasColorOffsets:Bool = false, ?blend:BlendMode, smooth:Bool = false, ?shader:FlxShader)
	{
		...
		return startTrianglesBatch(graphic, smooth, colored, blend, hasColorOffsets);
		#else
		var itemToReturn = null;
		#if !FLX_DRAW_QUADS
		var blendInt:Int = FlxDrawBaseItem.blendToInt(blend);
	...
	public function startTrianglesBatch(graphic:FlxGraphic, smoothing:Bool = false, isColored:Bool = false, ?blend:BlendMode, ?hasColorOffsets:Bool, ?shader:FlxShader):FlxDrawTrianglesItem
	{
		...
		if (_currentDrawItem != null
			...
			#end
			#if !flash && _headTriangles.hasColorOffsets == hasColorOffsets #end // && _headTriangles.shader == shader #end
		)
		{
	...
	@:allow(flixel.system.frontEnds.CameraFrontEnd)
	function render() {
		var item = _currentDrawStack?.next ?? _headOfDrawStack;
		while (item != null) {
			(_currentDrawStack = item).render(this);
			item = item.next;
		}
	}

	public function drawNote(?frame:FlxFrame, matrix:FlxMatrix, ?transform:ColorTransform, ?blend:BlendMode, ?smoothing:Bool = false, ?customColoring:Bool = false):Void
	{
	...
	public function drawNoteVertices(?frame:FlxFrame, vertices:Array<Float>, ?transform:ColorTransform, ?blend:BlendMode, ?smoothing:Bool = false, ?customColoring:Bool = false, ?stealth:Float, ?alpha:Float, ?stealthCol:Vector3):Void
	{
		if (FlxG.renderBlit) // don't feel like adding support for this
			return;
	...
	public function drawTriangles(graphic:FlxGraphic, vertices:DrawData<Float>, indices:DrawData<Int>, uvtData:DrawData<Float>, ?colors:DrawData<Int>, ?position:FlxPoint, ?blend:BlendMode, smoothing:Bool = false, ?transform:ColorTransform, ?shader:FlxShader):Void
	{
		if (FlxG.renderBlit)
		{
			if (position == null)
		...
		else
		{
			...
			#else
			var drawItem:FlxDrawTrianglesItem = startTrianglesBatch(graphic, smoothing, isColored, blend, false);
	...
	override public function destroy():Void
	{
		...
		// always clean the draw stack
		clearDrawStack();
		_storageTilesHead = null;
		_storageNoteHead = null;
		_storageTrianglesHead = null;
